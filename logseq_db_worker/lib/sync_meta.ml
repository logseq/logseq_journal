...
let create_full ~graph_id ~schema ~applied_server_t ~checksum ~status ~last_error =
  ...
  then Error "active sync metadata must not contain an error"
    ...
    status = Paused
    && (match last_error with None -> true | Some value -> String.length value = 0)
  then Error "paused sync metadata must contain an error"
  else if
    (match last_error with None -> false | Some value -> String.length value > 4096)
...
let advance metadata ~applied_server_t ~checksum =
  create_full
...
let initialize_database db metadata =
  let create_sql =
  ...
  | Ok () ->
    ...
    Fun.protect
      ...
      (fun () ->
         Sqlite3.Rc.check (Sqlite3.bind_int statement 1 metadata.format_version);
         Sqlite3.Rc.check
           (Sqlite3.bind_text statement 2 (Graph_types.Uuid.to_string metadata.graph_id));
         Sqlite3.Rc.check (Sqlite3.bind_int statement 3 metadata.schema.major);
         Sqlite3.Rc.check (Sqlite3.bind_int statement 4 metadata.schema.minor);
         Sqlite3.Rc.check (Sqlite3.bind_int statement 5 metadata.applied_server_t);
         Sqlite3.Rc.check (Sqlite3.bind_text statement 6 metadata.checksum);
         Sqlite3.Rc.check
           (Sqlite3.bind_text
              statement
              7
              (match metadata.status with Active -> "active" | Paused -> "paused"));
         Sqlite3.Rc.check
           (Sqlite3.bind
              statement
              8
              (match metadata.last_error with
      ...
      (fun () ->
         ...
         | Sqlite3.Rc.ROW ->
           ...
           let status =
             ...
             | Sqlite3.Data.TEXT "active" -> Ok Active
             | TEXT "paused" -> Ok Paused
      ...
      let statement =
        Sqlite3.prepare
          ...
          "UPDATE sync_meta SET format_version = ?, graph_id = ?, schema_major = ?, \
           schema_minor = ?, applied_server_t = ?, checksum = ?, status = ?, \
