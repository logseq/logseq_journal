...
let admission_error = function
  | Admission.Unsupported_schema ->
    error Error.Unsupported_schema "The graph schema is older than 65.33 or malformed."
...
let local_graph_uuid db =
  match single_kv db "logseq.kv/local-graph-uuid" with
  | Ok (Some value) -> uuid_of_value value
  | Ok None | Error () -> Error "missing local graph UUID"
;;

let remote_flag db =
  match single_kv db "logseq.kv/graph-remote?" with
  | Ok None -> Admission.Absent
  | Ok (Some (Datascript.Bool value)) -> Admission.Boolean value
  | Ok (Some _) | Error () -> Admission.Malformed "invalid remote flag"
;;

let has_non_nil_kv db ident =
  match single_kv db ident with
  | Ok (Some Datascript.Nil) | Ok None -> false
  | Ok (Some _) | Error () -> true
;;

let schema_has_attributes schema required =
  List.for_all (fun attr -> List.mem_assoc attr schema) required
;;

let startup_schema_valid schema =
  let required =
    [ "db/ident"
    ; "block/uuid"
    ; "block/title"
  ...
  in
  schema_has_attributes schema required
;;
...
type resolved_target =
  ...
  ; catalog : Snapshot.catalog
  ; kind : [ `Snapshot of Graph_types.Uuid.t | `Native ]
  }
...
let build_engine dependencies config target owner connection db storage =
  ...
  with
  | Error (), _, _, _ ->
    fail (error Error.Unsupported_schema "The graph schema is malformed.")
  | ( Ok schema
    , Ok local_graph_uuid
    , Ok (Some (Datascript.String "db"))
    , Ok startup_metadata ) ->
    let rtc_identity = has_non_nil_kv db "logseq.kv/graph-uuid" in
    let observed =
      Admission.
        { schema
        ; local_graph_uuid = Some local_graph_uuid
        ; remote_flag = remote_flag db
        ; rtc_graph_uuid = (if rtc_identity then Some local_graph_uuid else None)
        ; client_ops_graph_uuid = None
        ; codec_lossless = true
        ; required_schema_present = startup_schema_valid startup_metadata.schema
        }
    in
    (match Admission.admit observed with
     | Error admission -> fail (admission_error admission)
     | Ok admission_facts ->
       let basis = startup_metadata.basis in
       let session =
       ...
       let target_resources =
         ...
         | `Snapshot token ->
           Ok
             ...
             , Backup.create ~catalog:target.catalog ~source_token:token
        ...
        | Ok (write_target, backup, mode) ->
          Ok
            ...
            ; graph_info =
                ...
                ; basis
                ; mode
                ; admission_facts = admission_facts @ [ Graph_types.Ownership_verified ]
                }
  ...
  | _, Error _, _, _ | _, _, Error (), _ | _, _, Ok None, _ | _, _, Ok (Some _), _ ->
    fail (corrupt_storage ())
;;
      ...
      (match Ownership.acquire ~target:ownership_target ~graph_dir:target.graph_dir with
       ...
       | Ok owner ->
         (match target.kind with
          ...
          | `Snapshot _ -> open_owned dependencies config target owner)))
;;
...
let ios_native_fallback config =
  ...
  | Snapshot _ | Import_snapshot _ -> None
;;
...
let ensure_write_session t mutation_id =
  ...
  | None ->
    ...
    (match Backup.ensure_verified t.backup with
    ...
    then
      ...
      | Ok plan ->
        (match
         ...
         | Ok staged ->
           if
             mutation_requires_full_structure_validation mutation
             && not (tree_structurally_valid (Storage_session.staged_db_after staged))
           then
             Protocol.failed
               ~request_id
               ~phase:Execute
    ...
    then failed Error.Unsupported_api_version "The request API version is unsupported."
      ...
      let response =
        ...
        | Protocol.Read command ->
          (match
             Read_model.execute
               { db = Storage_session.current_db t.session
               ; basis
               ; now_ms = t.epoch_ms ()
