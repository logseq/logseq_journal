...
external decompress_gzip
  :  string
  -> string
  -> int
  -> int
  = "logseq_journal_gzip_decompress"
...
let decompress ~maximum_bytes source destination =
  remove_if_exists destination;
...
let peel_gzip_layers ~maximum_bytes ~source ~destination ~temporary_paths =
  if maximum_bytes <= 0
  then Error "snapshot decompressed bound must be positive"
    ...
    | first :: second :: _ ->
      ...
      (match gzip_signature source with
       ...
       | Ok true ->
         (match decompress ~maximum_bytes source first with
          | Error message -> fail message
          | Ok () ->
            (match gzip_signature first with
             ...
             | Ok true ->
               (match decompress ~maximum_bytes first second with
                | Error message -> fail message
                | Ok () ->
