...
let create ~code ~message ~details =
  ...
  else if List.length details > 64 || not (List.for_all (fun detail -> valid_detail_name detail.name) details)
...
let detail_to_yojson detail =
  let kind, value =
    ...
    | Detail_strings values -> "strings", `List (List.map (fun value -> `String value) values)
    | Detail_uuids values ->
        ( "uuids"
...
let detail_of_yojson = function
  | `Assoc [ ("name", `String name); ("value", `Assoc [ ("type", `String kind); ("value", value) ]) ] ->
      ...
      let parsed =
        ...
        | "int64", `String value ->
            (try Ok (Detail_int (Int64.of_string value)) with Failure _ -> Error "invalid int detail")
        | "bool", `Bool value -> Ok (Detail_bool value)
        | "uuid", `String value -> Result.map (fun value -> Detail_uuid value) (Graph_types.Uuid.of_string value)
        | "strings", value -> parse_strings (fun values -> Detail_strings values) value
        | "uuids", `List values ->
            let rec loop acc = function
              | [] -> Ok (Detail_uuids (List.rev acc))
              | `String value :: rest ->
                  Result.bind (Graph_types.Uuid.of_string value) (fun uuid -> loop (uuid :: acc) rest)
...
let of_yojson = function
  | `Assoc [ ("code", `String code); ("message", `String message); ("details", `List details) ] ->
      (match code_of_string code with
       | None -> Error "unknown error code"
       | Some code ->
           let rec parse acc = function
             | [] -> create ~code ~message ~details:(List.rev acc)
             | detail :: rest -> Result.bind (detail_of_yojson detail) (fun detail -> parse (detail :: acc) rest)
           in
           parse [] details)
