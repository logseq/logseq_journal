  ...
  Result.bind (ensure_root application_support_directory) (fun root ->
    ...
    then Ok None
      ...
      Result.bind (read_bounded path) (fun source ->
        try
          Result.bind
            (Sync_catalog.of_yojson (Yojson.Safe.from_string source))
            (fun cache ->
               if
                 String.equal (Sync_catalog.user_id cache) user_id
                 && String.equal (Sync_catalog.base_url cache) base_url
               then Ok (Some cache)
...
let save ~application_support_directory cache =
  Result.bind (ensure_root application_support_directory) (fun root ->
    let source = Sync_catalog.to_yojson cache |> Yojson.Safe.to_string in
    ...
    then Error "catalog cache exceeds its size bound"
      ...
      let destination =
        cache_path
          root
          ~user_id:(Sync_catalog.user_id cache)
          ~base_url:(Sync_catalog.base_url cache)
      in
