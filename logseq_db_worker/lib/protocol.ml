...
let read_to_json = function
  ...
  | Sync_status -> `Assoc [ "type", `String "syncStatus" ]
  | Sync_pending -> `Assoc [ "type", `String "syncPending" ]
...
let request_to_yojson request =
  let command =
    ...
    | Mutate mutation -> mutation_to_json mutation
    | Sync_receive { transport; payload } ->
      `Assoc
        [ "type", `String "syncReceive"
        ; ( "transport"
          , `String
...
let request_of_yojson json =
  try
    ...
    let command =
      ...
      else if String.equal kind "syncReceive"
      then (
        let fields = exact_assoc [ "type"; "transport"; "payload" ] command_json in
        let transport =
          match string (field "transport" fields) with
          | "websocket" -> Websocket
          | "httpPull" -> Http_pull
          | _ -> decode_error "invalid sync transport"
        in
        Sync_receive { transport; payload = string (field "payload" fields) })
      else Mutate (mutation_of_json kind command_json)
    in
...
let sync_state_string = function
  | Sync_active -> "active"
  | Sync_paused_state -> "paused"
;;

let sync_state = function
  | "active" -> Sync_active
  | "paused" -> Sync_paused_state
  | _ -> decode_error "invalid sync state"
;;

let sync_activity_string = function
  | Pull_applied -> "pullApplied"
  | Pull_duplicate -> "pullDuplicate"
  | Pull_required -> "pullRequired"
  | Sync_paused -> "syncPaused"
  | Sync_submission_blocked -> "submissionBlocked"
;;

let sync_activity = function
  | "pullApplied" -> Pull_applied
  | "pullDuplicate" -> Pull_duplicate
  | "pullRequired" -> Pull_required
  | "syncPaused" -> Sync_paused
  | "submissionBlocked" -> Sync_submission_blocked
  | _ -> decode_error "invalid sync activity"
;;

let optional_string_of_json = function
  | `Null -> None
  | value -> Some (string value)
;;

let sync_status_to_json (status : sync_status) =
  `Assoc
    [ "type", `String "syncStatus"
    ; "state", `String (sync_state_string status.state)
    ; "appliedServerT", `Int status.applied_server_t
    ; "checksum", `String status.checksum
    ; "lastError", option_json (fun value -> `String value) status.last_error
    ]
;;

let sync_status_of_json json =
  let fields =
    exact_assoc [ "type"; "state"; "appliedServerT"; "checksum"; "lastError" ] json
  in
  if string (field "type" fields) <> "syncStatus"
  then decode_error "unsupported sync status type";
  { state = sync_state (string (field "state" fields))
  ; applied_server_t = integer (field "appliedServerT" fields)
  ; checksum = string (field "checksum" fields)
  ; last_error = optional_string_of_json (field "lastError" fields)
  }
;;

let sync_pending_to_json (pending : sync_pending) =
  `Assoc
    [ "type", `String "syncPending"
    ; "payload", option_json (fun value -> `String value) pending.payload
    ; "count", `Int pending.count
    ; "blockedError", option_json (fun value -> `String value) pending.blocked_error
    ]
;;

let sync_pending_of_json json =
  let fields = exact_assoc [ "type"; "payload"; "count"; "blockedError" ] json in
  if string (field "type" fields) <> "syncPending"
  then decode_error "unsupported sync pending type";
  let count = integer (field "count" fields) in
  if count < 0 then decode_error "invalid pending count";
  { payload = optional_string_of_json (field "payload" fields)
  ; count
  ; blocked_error = optional_string_of_json (field "blockedError" fields)
  }
;;

let sync_success_to_json (result : sync_success) =
  `Assoc
    [ "type", `String "sync"
    ; "activity", `String (sync_activity_string result.activity)
    ; "state", `String (sync_state_string result.state)
    ; "appliedServerT", `Int result.applied_server_t
    ; "checksum", `String result.checksum
    ; "lastError", option_json (fun value -> `String value) result.last_error
    ; "mutation", option_json mutation_success_to_json result.mutation
    ]
;;

let sync_success_of_json json =
  let fields =
    exact_assoc
      [ "type"
      ; "activity"
      ...
      json
  in
  if string (field "type" fields) <> "sync" then decode_error "unsupported sync type";
  { activity = sync_activity (string (field "activity" fields))
  ; state = sync_state (string (field "state" fields))
  ; applied_server_t = integer (field "appliedServerT" fields)
  ; checksum = string (field "checksum" fields)
  ; last_error = optional_string_of_json (field "lastError" fields)
  ; mutation =
      (match field "mutation" fields with
       | `Null -> None
       | value -> Some (mutation_success_of_json value))
  }
;;
...
let success_to_json = function
  ...
  | Sync_status_result status -> sync_status_to_json status
  | Sync_pending_result pending -> sync_pending_to_json pending
  | Sync_result result -> sync_success_to_json result
  ...
  try
    ...
    | "succeeded" ->
      ...
      let success =
        ...
        | "syncStatus" -> Sync_status_result (sync_status_of_json success_json)
        | "syncPending" -> Sync_pending_result (sync_pending_of_json success_json)
        | "sync" -> Sync_result (sync_success_of_json success_json)
