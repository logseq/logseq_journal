...
let validate_integer value =
  ...
  String.iteri (fun index character -> if index > 0 then ignore (digit_index character)) value
...
let validate key =
  ...
  let fractional = String.sub key (String.length integer) (String.length key - String.length integer) in
  String.iter (fun character -> ignore (digit_index character)) fractional;
  if String.length fractional > 0 && Char.equal fractional.[String.length fractional - 1] zero
  then raise (Invalid "trailing zero")

let is_valid key =
  try validate key; true with
  | Invalid _ -> false
...
let rec midpoint lower upper =
  ...
  (match upper with
   | Some upper when String.length upper > 0 && Char.equal upper.[String.length upper - 1] zero ->
  ...
  let common =
    ...
    | Some upper ->
        ...
        let rec loop index =
          if index = limit || not (Char.equal lower.[index] upper.[index]) then index else loop (index + 1)
        in
        loop 0
  in
  if common > 0
  then
    ...
    let upper = Option.map (fun value -> String.sub value common (String.length value - common)) upper in
    ...
    then String.make 1 digits.[(lower_digit + upper_digit + 1) / 2]
      ...
      | _ ->
          let rest = if String.length lower = 0 then "" else String.sub lower 1 (String.length lower - 1) in
...
let generate lower upper =
  ...
  (match lower, upper with
   | Some lower, Some upper when String.compare lower upper >= 0 -> raise (Invalid "lower >= upper")
   | _ -> ());
  match lower, upper with
  ...
  | Some lower, None ->
      ...
      let fractional = String.sub lower (String.length integer) (String.length lower - String.length integer) in
      (match increment_integer integer with
       | Some value -> value
       | None -> integer ^ midpoint fractional None)
  | Some lower, Some upper ->
      let lower_integer = integer_part lower in
      let lower_fraction = String.sub lower (String.length lower_integer) (String.length lower - String.length lower_integer) in
      let upper_integer = integer_part upper in
      let upper_fraction = String.sub upper (String.length upper_integer) (String.length upper - String.length upper_integer) in
