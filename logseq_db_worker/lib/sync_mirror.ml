...
let resolve ~application_support_directory ~graph_id =
  ...
  | Ok _ ->
    ...
    then Error Mirror_missing
      ...
      match Sync_meta.read_path database_path with
...
let insert_row statement (row : Sync_snapshot.row) =
...
let import_snapshot_rows database_path snapshot_path expected_rows =
  ...
  then Error (Invalid_snapshot "snapshot file is missing or unsafe")
    ...
    try
      ...
      in
      let parser = Sync_snapshot.create_parser ~max_frame_bytes:(64 * 1024 * 1024) in
      let state = Sync_snapshot.create_import ~expected_rows in
      ...
      let rec read () =
        ...
        | 0 -> Ok ()
        | count ->
          (match Sync_snapshot.feed parser (Bytes.sub_string buffer 0 count) with
           | Error message -> Error message
           | Ok rows ->
             (match Sync_snapshot.accept_rows state rows with
              | Error _ as error -> error
              | Ok () ->
      ...
      let imported =
        Fun.protect
          ...
          (fun () ->
             ...
             | Error _ as error -> error
             | Ok () ->
               (match Sync_snapshot.finish parser with
                | Error _ as error -> error
                | Ok () -> Sync_snapshot.finish_import state |> Result.map (fun _ -> ())))
      in
      match imported with
...
let plaintext_snapshot_db decrypt db =
  let rec loop datoms = function
    ...
    | datom :: rest when List.mem datom.Datascript.a Sync_e2ee.protected_attributes ->
      (match datom.v with
...
let initialize_metadata database_path metadata =
  ...
  try
    Sqlite3.Rc.check (Sqlite3.exec db "BEGIN IMMEDIATE");
    match Sync_meta.initialize_database db metadata with
    | Error message ->
...
let finalize_computed_checksum ?expected_checksum database_path metadata =
  ...
  | Ok connection ->
    ...
    (match Logseq_sqlite_storage.restore_database connection with
     ...
     | Ok db ->
       let checksum = Sync_checksum.recompute ~e2ee:(Sync_checksum.graph_e2ee db) db in
       if
