...
let connect ~sw ~environment ~uri ~token ~maximum_frame_bytes ~on_message ~on_close =
  initialize_rng ();
  if maximum_frame_bytes <= 0
  then Error "WebSocket frame bound must be positive"
  else
    bind (open_flow ~sw ~environment uri) (fun flow ->
      ...
      try
        let descriptor =
          Eio.Time.with_timeout_exn (Eio.Stdenv.clock environment) 30. (fun () ->
...
let send t payload =
  ...
  else if String.length payload > Protocol.maximum_request_bytes
