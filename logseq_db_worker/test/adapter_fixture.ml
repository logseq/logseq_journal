...
let create_oracle_graph root graph_name =
  ...
  |> List.iter (fun row ->
    ...
    Sqlite3.Rc.check (Sqlite3.bind_text statement 2 (row |> member "content" |> to_string));
    Sqlite3.Rc.check
      (Sqlite3.bind
         statement
         3
...
let missing_config support =
  config support (uuid "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee")
;;
