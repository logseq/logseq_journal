type mode =
  | Runtime_flow
  | Runtime_flow_with_pagination
  | Runtime_flow_with_persistence_failure

type generated =
  { support_root : string
  ; graph_id : Logseq_db_types.Graph_types.Uuid.t
  ; graph_dir : string
  ; user_id : string
  ; base_url : string
  ; expected_timeline_text : string
  }

val create : support_root:string -> mode:mode -> (generated, string) result
val to_yojson : generated -> Yojson.Safe.t
val create_unencrypted_warm_start : support_root:string -> (generated, string) result
val create_encrypted_warm_start : support_root:string -> (generated, string) result
val managed_to_yojson : generated -> Yojson.Safe.t
