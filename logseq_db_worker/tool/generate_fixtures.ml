...
let parse arguments =
  ...
  | [ mode; "--support-root"; support_root ] ->
    (match mode with
     ...
     | "runtime-flow" -> Ok (`Managed Generator.Runtime_flow, support_root)
     | "runtime-flow-pagination" ->
       Ok (`Managed Generator.Runtime_flow_with_pagination, support_root)
     | "runtime-flow-failure" ->
       Ok (`Managed Generator.Runtime_flow_with_persistence_failure, support_root)
     | _ -> Error usage)
  | _ -> Error usage
;;
