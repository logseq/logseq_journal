...
(* Scenario: A completed mirror inspection is replayed after graph attachment.
   ...
   WebSocket token owner for its legitimate completion. *)
let run () =
  ...
  in
  let follow_up =
    Core.Token_provided (fixture.websocket_token, "replacement-websocket-token")
  in
  let accepted =
  ...
  | _ -> Alcotest.fail "BC02 current graph token owner was not retained"
;;
