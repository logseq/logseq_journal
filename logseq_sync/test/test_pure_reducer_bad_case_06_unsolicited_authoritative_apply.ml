(* Scenario: Authoritative_batch_applied arrives without an active authoritative owner.
   Expected: the reducer ignores the unsolicited worker callback. *)
...
let checkpoint applied_server_t =
...
let opened_graph () =
  ...
  let catalog_token =
    ...
    | _ -> fail "BC06 setup authentication emitted unexpected effects"
  in
  let authorized =
  ...
  let catalog_completion =
    ...
    | _ -> fail "BC06 setup catalog authorization emitted unexpected effects"
  in
  ...
  let mirror_request =
    ...
    | _ -> fail "BC06 setup graph selection emitted unexpected effects"
  in
  let open_request : Core.graph_open_request =
    { graph = mirror_request.graph
    ; graph_directory = "/worker/bad-case-06"
    ; database_path = "/worker/bad-case-06/db.sqlite"
  ...
  let attach_request =
    ...
    | _ -> fail "BC06 setup mirror inspection emitted unexpected effects"
  in
  let attached =
    Core.step
      inspected.next
  ...
  let websocket_token =
    ...
    | _ -> fail "BC06 setup graph attachment emitted unexpected effects"
  in
  let connecting =
  ...
  let connection =
    ...
    | _ -> fail "BC06 setup WebSocket authorization emitted unexpected effects"
  in
  ...
  (match opened.effects with
   ...
   | _ -> fail "BC06 setup WebSocket opening emitted unexpected effects");
  opened.next
;;

let test_unsolicited_authoritative_apply_is_ignored () =
  let origin = opened_graph () in
...
let () =
  Alcotest.run
    "pure reducer bad case 06"
    [ ( "pure core bad case"
      , [ Alcotest.test_case
            "unsolicited authoritative apply is ignored"
            `Quick
            test_unsolicited_authoritative_apply_is_ignored
        ] )
    ]
;;
