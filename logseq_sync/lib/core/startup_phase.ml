...
let failure_receipt (type kind) (request : kind local_request) reason
...
module Local_completion = struct
  ...
  let wrapped_graph_key_failed (request : wrapped_graph_key local_request) ~diagnostic:_ =
    failure_receipt request Wrapped_graph_key_unavailable
  ;;

  let local_private_key_failed (request : local_private_key local_request) ~diagnostic:_ =
