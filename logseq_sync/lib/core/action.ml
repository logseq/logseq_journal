type local
type network
type account = Startup_phase.account
type graph_level = Startup_phase.graph
type connection = Startup_phase.connection
...
type network_scope =
  | Account_scope of Startup_phase.account_scope_view
  | Graph_scope of Startup_phase.graph_scope_view
  | Connection_scope of Startup_phase.connection_scope_view
...
type inspect_mirror =
  { request : Startup_phase.mirror Startup_phase.local_request
  ; graph : graph
  }

type wrapped_key_lookup =
  { wrapped_key_request : Startup_phase.wrapped_graph_key Startup_phase.local_request
  ; private_key_request : Startup_phase.local_private_key Startup_phase.local_request
  }

type wrapped_key_save =
  { scope : Startup_phase.graph_scope_view
  ; encrypted_graph_key : wrapped_graph_key
  }

type graph_cleanup = { scope : Startup_phase.graph_scope_view }
type account_cleanup = { scope : Startup_phase.account_scope_view }

type open_graph =
  { request : Startup_phase.graph_open Startup_phase.local_request
  ; graph : graph
...
type connection_token =
  { scope : Startup_phase.connection_scope_view
  ; token : string
  }
...
type _ t =
  ...
  | Fetch_catalog :
      { scope : Startup_phase.account_scope_view
      ; token : string
      }
      -> network t
...
let classify (Pack action) =
  ...
  | ( Need_id_token _
    ...
    | Apply_sync_frame _
    | Recover_submitted _ ) as action -> Network_action action
;;
...
let valid_graph (graph : graph) =
  bounded_text ~maximum:1_024 graph.name
  && graph.schema.major >= 0
  && graph.schema.minor >= 0
;;

let valid_token = bounded_text ~maximum:65_536
let valid_payload = bounded_text ~maximum:(4 * 1_024 * 1_024)

let graph_matches_scope graph scope =
  String.equal
    (Graph_types.Uuid.to_string graph.Logseq_db_types.Managed_graph.graph_id)
    scope.Startup_phase.graph_id
;;
...
let load_and_verify_wrapped_graph_key wrapped_key_request private_key_request =
  if
    Startup_phase.local_request_graph_scope wrapped_key_request
    = Startup_phase.local_request_graph_scope private_key_request
...
let verify_and_save_wrapped_graph_key scope encrypted_graph_key =
  Verify_and_save_wrapped_graph_key
    { scope = Startup_phase.graph_scope_view scope; encrypted_graph_key }
;;

let delete_wrapped_graph_key scope =
  Delete_wrapped_graph_key { scope = Startup_phase.graph_scope_view scope }
;;

let delete_account_secrets scope =
  Delete_account_secrets { scope = Startup_phase.account_scope_view scope }
;;
...
let delete_mirror scope = Delete_mirror { scope = Startup_phase.graph_scope_view scope }

let need_id_token
  : type level.
    level Startup_phase.network_permit
    -> challenge_id:string
    -> level auth_purpose
  ...
  then Error `Invalid_payload
    ...
    let purpose, scope =
      ...
      | Catalog_discovery ->
        ( Catalog_discovery_name
        , Account_scope
            (Startup_phase.permit_account_scope permit |> Startup_phase.account_scope_view)
        )
      | Snapshot_bootstrap ->
        ( Snapshot_bootstrap_name
        , Graph_scope
            (Startup_phase.permit_graph_scope permit |> Startup_phase.graph_scope_view) )
      | E2ee_key_access ->
        ( E2ee_key_access_name
        , Graph_scope
            (Startup_phase.permit_graph_scope permit |> Startup_phase.graph_scope_view) )
      | Websocket_connect ->
        ( Websocket_connect_name
        , Connection_scope
            (Startup_phase.permit_connection_scope permit
             |> Startup_phase.connection_scope_view) )
    in
...
let graph_request constructor permit graph ~token =
  let scope = Startup_phase.permit_graph_scope permit |> Startup_phase.graph_scope_view in
  if valid_graph graph && valid_token token && graph_matches_scope graph scope
  then Ok (constructor { scope; graph; token })
  else
    Error
      (if valid_graph graph && valid_token token
       then `Scope_mismatch
       else `Invalid_payload)
;;

let fetch_snapshot_baseline =
  graph_request (fun request -> Fetch_snapshot_baseline request)
;;

let fetch_snapshot_metadata =
  graph_request (fun request -> Fetch_snapshot_metadata request)
;;
...
let connection_scope permit =
  Startup_phase.permit_connection_scope permit |> Startup_phase.connection_scope_view
;;
