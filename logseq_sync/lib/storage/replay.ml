...
let decode_transactions decrypt_protected db txs =
  let rec loop decoded = function
    | [] -> Ok (List.rev decoded)
    | (tx : Protocol.pull_tx) :: rest ->
      (match Tx.decode ?decrypt_protected ~db tx.tx with
...
let pause before_commit session metadata ~basis ~remote_t ~local_checksum ~remote_checksum
  ...
  let message =
    Printf.sprintf
      ...
      remote_checksum
  in
  match State.pause metadata ~message with
  ...
  | Ok transactions ->
    (match
     ...
     | Ok staged ->
       ...
       let local_checksum =
         Checksum.recompute ~e2ee:(Checksum.graph_e2ee db_after) db_after
       in
       if
         match checksum with
       ...
       then
         ...
         match State.advance metadata ~applied_server_t:t ~checksum with
         | Error message -> Error (Protocol_error message)
         | Ok metadata ->
...
let apply_pull
      ...
      message
  =
  match metadata.Sync_checkpoint.status with
  | Paused ->
    Error
      (Sync_paused (Option.value metadata.last_error ~default:"persistent sync error"))
  | Active ->
    (match message with
     | Protocol.Pull_ok { t; checksum; txs } ->
       (match
          Protocol.validate_pull_continuity ~applied_t:metadata.applied_server_t message
        with
        | Error message -> Error (Protocol_error message)
        | Ok () ->
