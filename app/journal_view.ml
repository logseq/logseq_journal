...
module Text_editing = struct
  module Range = struct
    ...
    let equal left right = left.start_utf16 = right.start_utf16 && left.end_utf16 = right.end_utf16
  end
  ...
  module Utf16 = struct
    let length s =
      ...
      while !i < n do
        ...
        let advance, units =
          if byte < 0x80 then 1, 1
          else if byte land 0xE0 = 0xC0 then 2, 1
          else if byte land 0xF0 = 0xE0 then 3, 1
          else if byte land 0xF8 = 0xF0 then 4, 2
          else 1, 1
        in
        i := !i + advance;
        count := !count + units
...
let text ?key ?(style : Style.Text_style.t option) ?text_align:_ ?line_limit:_
        ?truncation:_ value
  =
...
let symbol ?key ?size ?color ?rendering:_ ~name () =
  element ?key (fun context parent ->
    ...
    Option.iter (fun size -> Lui_ui.size context node (string_of_int (int_of_float_nan size))) size;
...
let column ?key ?(spacing = 16.) ?(alignment = Layout.Horizontal_alignment.Center) children =
  element ?key (fun context parent ->
...
let frame
  ...
  modify
    (fun context node ->
       Option.iter
         (fun v -> Lui_ui.width context node (int_of_float_nan v))
         width;
       Option.iter
         (fun v -> Lui_ui.height context node (int_of_float_nan v))
         height;
       Option.iter
       ...
       Option.iter
         (fun limit ->
            apply_frame_limit context node Lui_protocol.MinWidth Lui_protocol.MaxWidth
              limit)
         max_width;
       Option.iter
         (fun limit ->
            apply_frame_limit context node Lui_protocol.MinHeight Lui_protocol.MaxHeight
              limit)
         max_height)
    t
  |> fun result -> (match frame_key with Some key -> { result with key = Some key } | None -> result)
;;

let padding ?key:_ ~insets t =
  modify
    (fun context node ->
       Lui_ui.padding context node (int_of_float_nan insets))
    t
;;
...
let background ?key:_ ?corner_radius ~color t =
  modify
    (fun context node ->
       ...
       Option.iter (fun radius -> Lui_ui.corner_radius context node (int_of_float_nan radius)) corner_radius)
    t
;;
...
let button
  ...
  element ?key (fun context parent ->
    ...
    Option.iter (fun style -> Lui_ui.string_property context node Lui_protocol.VariantValue (Button_style.variant style)) style;
    (match role with
     | Button_role.Normal -> ()
     | role -> Lui_ui.string_property context node Lui_protocol.VariantValue (Button_role.variant role));
...
let content_unavailable ?key ~label ?description ?actions () =
  element ?key (fun context parent ->
    ...
    Lui_ui.main context node "center";
    (match parent with
     | Some parent -> Lui_ui.append context parent node
     | None -> ());
    ignore (label.mount context (Some node));
    Option.iter (fun description -> ignore (description.mount context (Some node))) description;
    Option.iter (fun actions -> ignore (actions.mount context (Some node))) actions;
    node)
;;
...
module Section = struct
  let create ?key ?header ?footer entries =
    element ?key (fun context parent ->
      let node = Lui_ui.panel context in
      (match parent with
       | Some parent -> Lui_ui.append context parent node
       | None -> ());
      Option.iter (fun header -> ignore (header.mount context (Some node))) header;
      List.iter (fun (entry : Keyed.t) -> ignore (entry.view.mount context (Some node))) entries;
      Option.iter (fun footer -> ignore (footer.mount context (Some node))) footer;
      node)
  ;;
end
...
module Toolbar = struct
  ...
  let item ~key ?placement content = { item_key = key; placement; content; spacing = None; is_group = false }
  ...
  let spacer ~key ?placement:_ _spacing =
    { item_key = key
    ; placement = None
    ; content = element (fun context parent ->
        let node = Lui_ui.spacer context in
        (match parent with
         | Some parent -> Lui_ui.append context parent node
         | None -> ());
  ...
  let create ?key ~items t =
    element ?key (fun context parent ->
      ...
      (match parent with
       | Some parent -> Lui_ui.append context parent node
       | None -> ());
      ignore ((mount_items items).mount context (Some node));
      ignore (t.mount context (Some node));
      node)
  ;;
end
...
module Scroll = struct
  ...
  let vertical ?key ?on_scroll:_ ?shows_indicators:_ ?fill_viewport:_ ?initial_anchor:_ t =
    element ?key (fun context parent ->
...
module Swipe_actions = struct
  ...
  let action ~key ?(enabled = true) ?(role = Button_role.Normal) ?symbol ~side ~title
            ~background ~on_press ()
    =
    ...
    element ?key:view.key (fun context parent ->
      ...
      List.iter
        (fun (action : action) ->
           ...
           Option.iter
             (fun symbol -> ignore (Lui_ui.append context item (Lui_ui.icon context symbol)))
             action.symbol;
    ...
    element ?key:view.key (fun context parent ->
      (match request with
       ...
       | Some request ->
         ...
         Option.iter
           (fun message -> Lui_ui.string_property context dialog Lui_protocol.DescriptionValue message)
           request.message;
         ...
         List.iter
           (fun (action : action) ->
              ...
              (match action.role with
               | Button_role.Normal -> ()
               | role ->
                 Lui_ui.string_property context item Lui_protocol.VariantValue
...
module Native_list = struct
  ...
  let disclosure_row ~key ?test_id ?(separator = Automatic) ?swipe_actions ?context_menu
                    ~expanded ~on_expanded_changed ~label children
    =
    { key
    ; test_id
    ; separator
                  ...
                  `Assoc
                    ...
                    ; ( "symbol"
                      , match a.symbol with
                        | Some s -> `String s
                        | None -> `Null )
                    ; "side", `String (match a.side with Start -> "start" | End -> "end")
            ...
            (List.map
               (fun (a : Context_menu.action) ->
                  `Assoc
                    [ "key", `String a.key
                    ; "enabled", `Bool a.enabled
                    ; "role", `String (match a.role with Normal -> "normal" | Destructive -> "destructive")
                    ; ( "symbol"
                      , match a.symbol with
                        | Some s -> `String s
                        | None -> `Null )
  ...
  let build sections ~style ~scroll_request ~track_visible ~track_scroll =
    let contents = ref [] in
    let push element = contents := !contents @ [ element ]; List.length !contents - 1 in
              ...
              `Assoc
                ...
                ; ( "target"
                  , `Assoc
                      ...
                      ; "row_path", `List (List.map (fun key -> `String key) request.target.row_path)
                      ] )
                ; ( "anchor"
         ...
         | Some (`String "scroll_completed") ->
           Option.iter
             (fun handler ->
                ...
                invoke
                  handler
                  (Event.Payload.Native_list_completion { token; outcome }))
             on_scroll_completed
...
module Navigation_stack = struct
  ...
  let destination ~page_key:_ ~title:_ ~can_pop:_ content = content

  (* The lui widget set has no navigation-stack node.  The router stays in the
     model: the topmost destination renders, and interactive pops arrive as
     [Navigation_path_changed] through the back affordance the shim renders. *)
  let create ?key ~title:_ ~on_path_change ~path root =
    element ?key (fun context parent ->
      let node = Lui_ui.column context in
      (match parent with
       | Some parent -> Lui_ui.append context parent node
      ...
      (match List.rev path with
       | [] -> ignore (root.mount context (Some node))
       | top :: _ ->
         let back =
           element (fun context parent ->
             let node = Lui_ui.button context in
             ...
             Lui_ui.on_event context node (fun event ->
               if is_press event
               then
                 invoke on_path_change (Event.Payload.Navigation_path_changed []));
             (match parent with
              | Some parent -> Lui_ui.append context parent node
              | None -> ());
             node)
         in
         ignore (back.mount context (Some node));
         ignore (top.mount context (Some node)));
      node)
  ;;
end
...
module Picker = struct
  ...
  let create ?key ?label:_ ?(style = Automatic) ?(enabled = true) ~selected_id
             ~on_select choices ()
    =
    element ?key (fun context parent ->
      let node =
...
module Menu = struct
  ...
  let submenu ~id ~label ?(enabled = true) entries = Submenu { id; label; enabled; entries }
      ...
      let rec mount_entry parent (entry : entry) =
        ...
        | entry ->
          ...
          (match entry with
           ...
           | Choice { id; label; selected; enabled } ->
             if not enabled then Lui_ui.disabled context item true;
             if selected then Lui_ui.bool_property context item Lui_protocol.Checked true;
             Lui_ui.on_event context item (fun event ->
               if is_press event then invoke on_select (Event.Payload.Int64 id));
             ignore (label.mount context (Some item))
...
module Native_widget = struct
  ...
  let decode extension event =
    extension.Extension.decode_event
      ~event_id:(Journal_ids.Native_widget.Event_id.of_int
                   event.Journal_lui_native.event_id)
