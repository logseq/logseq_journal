...
let startup_error stage message =
  let owner, recovery =
    match stage with
    | Logseq_sync.Api.During_authentication -> Authentication, Some Sign_in
...
let graph_matches snapshot (graph : Logseq_db_worker.graph_state) =
  graph.generation = snapshot.Logseq_sync.Api.startup.graph_generation
  && Option.equal
...
let derive ~snapshot ~(graph : Logseq_db_worker.graph_state) =
  let facts = snapshot.Logseq_sync.Api.startup in
